...
int main()
{
	...
	float *adata=malloc(sizeof(float)*size*size);
	float *bdata=malloc(sizeof(float)*size*size);
	float *cdata=malloc(sizeof(float)*size*size);
	FILE *myfile;
	...
	{
		double x = drand(1,100);		
		adata[i]=x;
		double y = drand(1,100);		
		bdata[i]=y;
	}*/
